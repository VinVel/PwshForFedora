Name:           dotnet-json-pointer-net-devel
Version:        5.3.1
Release:        1%{?dist}
Summary:        NuGet development package for JsonPointer.Net
License:        MIT
...
Requires:       dotnet-json-pointer-net%{?_isa} = %{version}-%{release}
BuildArch:      noarch

%description
The locally source-built NuGet package for applications compiling against
dotnet-json-pointer-net. It is intended for an offline NuGet feed and
contains no downloaded binary NuGet artifact.
...
dotnet build src/JsonPointer/JsonPointer.csproj --no-restore \
  ...
  -p:EnablePackageValidation=false
dotnet pack src/JsonPointer/JsonPointer.csproj --no-restore \
  --configuration Release --output "%{_builddir}/nuget-feed" \
  -p:PackageVersion=%{version} -p:TargetFrameworks=net8.0 \
  -p:EnablePackageValidation=false

%install
install -Dpm0644 "%{_builddir}/nuget-feed/JsonPointer.Net.%{version}.nupkg" \
  %{buildroot}%{_datadir}/dotnet/nuget/%{name}/JsonPointer.Net.%{version}.nupkg

%check
test -s "%{buildroot}%{_datadir}/dotnet/nuget/%{name}/JsonPointer.Net.%{version}.nupkg"

%files
%license LICENSE
%{_datadir}/dotnet/nuget/%{name}/JsonPointer.Net.%{version}.nupkg
